...
try {
    ...
    async function GetReleaseRules(firstMessage) {
        ...
        for (let i = 0; i < Rules["releaseRules"].length; i++) {
            if (Rules["releaseRules"][i]["tag"] === firstMessage ||
                ...
                return Rules["releaseRules"][i]["release"]
            }
    ...
    async function GetLastVersionTags(Patch) {
        let per_page = 100
        let page = 1
        let tagsList = GetTags(per_page, page)
        let getVersionTags = GetVersionTags(tagsList, 0)
        ...
        while (result === "0") {
            ...
            for (let i = 0; i < tagsList.length; i++) {
                result = getVersionTags = GetVersionTags(tagsList[i]['name'], Patch)
                if (result !== "0") {
                    return result
                }
            }
    ...
    async function MessageProcessing(message) {
        ...
        for (let i = 0; i < messageList.length; i++) {
            ...
            let firstMessage = messageList[i].split(" ")[0]
            let patch = messageList[i].split(" ")[1]

            let release = GetReleaseRules(firstMessage)
            break
            //console.log("release: ", release);

            //let version = GetLastVersionTags(patch)
            //console.log("version: ", version);

            //let ReleasePatchVersion = GetPlusVersion(release, patch, version)
