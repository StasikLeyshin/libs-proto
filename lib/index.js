...
try {
    ...
    (async() => {
        ...
        let filesCommit = await GetFilesCommit(sha)
        let resList = await GetPaths(filesCommit)
        await FilesCommitProcessing(resList, message, sha)
    })();
    ...
    async function CreateTag(tag, patch, sha){
        let createTag = await client.request('POST /repos/{owner}/{repo}/git/tags', {
            ...
            repo: REPO,
            tag: patch + "v" + tag,
            message: "To copy version: " +
                "```" +
                "github.com/StasikLeyshin/libs-proto/" + patch.slice(0, -1) + " v" + tag +
                "```",
            object: sha,
            type: 'commit',
            tagger: {
    ...
    async function FilesCommitProcessing(filesCommit, messageCommit, sha) {
        let firstMessage = String(messageCommit.split(" ")[0])
        ...
        for (let i = 0; i < filesCommit.length; i++) {
            let version = await GetLastVersionTags(filesCommit[i])

            if (version === "0") {
                version = InitialVersion
            } else {

                version = await GetPlusVersion(release, filesCommit[i], version)
                console.log("ReleasePatchVersion: ", version);
            }

            let res = await CreateTag(version, filesCommit[i], sha)

            console.log("CreateTag: ", res);

            res = await CreateRelease(version, filesCommit[i])
    ...
    async function GetPaths(filesList) {
        let resList = []
        let isResList = {}
        for (let i = 0; i < filesList.length; i++) {
            let strReplace = String(filesList[i]["filename"]).match(/grpc\/[-a-z]+\/pb\/(?=\w+\.pb\.go)|grpc-gateway\/[-a-z]+\/pb\/(?=\w+\.pb\.go)/gm)

            if (strReplace && filesList[i]["status"] !== "removed") {
                let filename = strReplace[0]
                let filenameNew =  String(filename).replace("/pb", "")
                if (isResList[filenameNew]) {
                    continue
                } else {
                    isResList[filenameNew] = 1
                }
                resList.push(filenameNew)
                //console.log(filenameNew)
            } else {
                let strReplace = String(filesList[i]["filename"]).match(/grpc-gateway\/[-a-z]+\/grpc_gateway_pb\/(?=\w+\.pb\.go)/gm)
                if (strReplace && filesList[i]["status"] !== "removed") {
                    let filename = strReplace[0]
                    let filenameNew = String(filename).replace("/grpc_gateway_pb", "")
                    if (isResList[filenameNew]) {
                        continue
                    } else {
                        isResList[filenameNew] = 1
